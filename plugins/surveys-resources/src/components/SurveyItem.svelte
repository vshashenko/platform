...
{#if inline}
  ...
  <span
    style={`--survey-color:${getPlatformColor(survey?.color ?? element?.color ?? 0, $themeStore.dark)}`}
    class="survey-item-inline overflow-label max-w-40"
    on:click
    use:tooltip={{
...
{:else}
  ...
  <div
    class="survey-item"
    style={`${getSurveyStyle(getPlatformColorDef(survey?.color ?? element?.color ?? 0, $themeStore.dark), selected)}`}
    on:click
    on:keydown
    use:tooltip={{
      label: surveys.string.SurveyTooltip,
  ...
  >
    ...
    {#if survey && surveyIcon && schema !== '0'}
      <span class="ml-1">
        <Icon icon={surveyIcon} size={'small'} />
      </span>
    {/if}
    {#if action}
