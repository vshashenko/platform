...
<script lang="ts">
  ...
  async function applyPlan(): Promise<void> {
    ...
    for (const item of searchPlanElements) {
      ...
      if (item.move.length > 0) {
        // We need to find all objects and add new tag elements to them and preserve form level
  ...
  function exportExpertSurveys(): string {
    // Construct csv
    const csv: string[] = []
...
<Card
  label={getEmbeddedLabel('Surveys optimizer')}
  okAction={applyPlan}
...
>
  ...
  <svelte:fragment slot="footer">
    ...
    />
    <Button
      label={getEmbeddedLabel('Export expert surveys')}
      on:click={() => {
        exportCSV('experts', exportExpertSurveys())
      }}
    />
    <Button
