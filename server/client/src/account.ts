...
export async function getPendingWorkspace (token: string, region: string, version: Data<Version>): Promise<BaseWorkspaceInfo | undefined> {
  const accountsUrl = getMetadata(plugin.metadata.Endpoint)
  if (accountsUrl == null) {
    throw new PlatformError(unknownError('No endpoint specified'))
  }
...
export async function updateWorkspaceInfo (token: string, workspaceId: string, event: 'progress' | 'create-done' | 'upgrade-done', progress: number): Promise<void> {
  const accountsUrl = getMetadata(plugin.metadata.Endpoint)
  if (accountsUrl == null) {
    throw new PlatformError(unknownError('No endpoint specified'))
  }
  await (
    await fetch(accountsUrl, {
      ...
      body: JSON.stringify({
        ...
        params: [token, workspaceId, event, progress]
      })
    })
  ).json()
}
